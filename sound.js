function searchsongs() {
		hi.style.filter = "blur(4px)";
		searchsong.style.display = "flex";
}
var s11 = document.getElementById("song11");
var s22 = document.getElementById("song22");
var s33 = document.getElementById("song33");
var s44 = document.getElementById("song44");
var	s55 = document.getElementById("song55");
var s66 = document.getElementById("song66");
var s77 = document.getElementById("song77");
var s88 = document.getElementById("song88");
var s99 = document.getElementById("song99");
var s00 = document.getElementById("song00");
var s12 = new Audio("../League-of-Introverts/pj song1/to the bone - pamungkas (guitar cover).mp3")
var s13 = new Audio("../League-of-Introverts/pj song1/Pone Yape - မမြင့်.mp3")
var s14 = new Audio("../League-of-Introverts/pj song1/ONLY - LEEHI (guitar cover).mp3")
var s15 = new Audio("../League-of-Introverts/pj song1/အချစ်အယူခံ (ခင်မောင်တိုး).mp3")
var s16 = new Audio("../League-of-Introverts/pj song1/မောင့်ဆေးကျောင်းသူ_-_ ခင်မောင်ထူး_-_.mp3")
var s17 = new Audio("../League-of-Introverts/pj song1/ဒီလိုဆိုမချစ်ဘူးကွယ် - ၀င်းဦး.mp3")
var s18 = new Audio("../League-of-Introverts/pj song1/ချစ်ရဲ့လားလို့ ခဏခဏမေးတဲ့ မောင့်ကိုပင်.mp3")
var s19 = new Audio("../League-of-Introverts/pj song1/ရုန်းတော်လွှတ်တော်-အေဝမ်းတင်မောင် & မေရှင်.mp3")
var s20 = new Audio("../League-of-Introverts/pj song1/cant help falling in love x perfect (elvis, ed sheeran) - guitar fingerstyle cover.mp3")
var s21 = new Audio("../League-of-Introverts/pj song1/Htoo eain thin - Chit Thu Lat Saung - ချစ်သူ့ လက်ဆောင်.mp3")
var s23 = new Audio("../League-of-Introverts/pj song1/at my worst - pink sweat$ (guitar fingerstyle cover).mp3")
var s24 = new Audio("../League-of-Introverts/pj song1/Until I Found You - Stephen Sanchez - Cover (Violin).mp3")
var s25 = new Audio("../League-of-Introverts/pj song1/life goes on - bts (guitar fingerstyle cover).mp3")
var s26 = new Audio("../League-of-Introverts/pj song1/ချစ်သူလမ်းကြား - စိုင်းအောင်ထီးခမ်း.mp3")
var s27 = new Audio("../League-of-Introverts/pj song1/ရေဒီယိုကမာရွတ်-Radio Kamaryut.mp3")
var s28 = new Audio("../League-of-Introverts/pj song1/မင်းသိဖို့ကောင်းတယ်  ဇော်ဝင်းရှိန် ၁၉၈၇.mp3")
var s29 = new Audio("../League-of-Introverts/pj song1/မိုးမိုး - ပြန်တွေးကြည့်.mp3")
var s30 = new Audio("../League-of-Introverts/pj song1/အချစ်မျက်ဝန်း - Lလွန်းဝါ (lyrics).mp3")
var s31 = new Audio("../League-of-Introverts/pj song1/ဒူးထောက်ပြီ - ဝိုင်ဝိုင်း.mp3")
var s32 = new Audio("../League-of-Introverts/pj song1/အေးချမ်းမေ -တစ်စစီကျိုးပဲ့နေတယ်.mp3")
var s34 = new Audio("../League-of-Introverts/pj song1/Soe Thu - ကြည်ဖြူပါတော့ (ကြည်ဖြူပါ မေရယ်).mp3")
var s35 = new Audio("../League-of-Introverts/pj song1/Shwe Mann Thu - Khin Maung Htoo.mp3")
var s36 = new Audio("../League-of-Introverts/pj song1/ငွေသော်တာရောင်ခြည် (ဆရာမ) ဘိုဘိုဟန်.mp3")
var s37 = new Audio("../League-of-Introverts/pj song1/gone - rosé (guitar fingerstyle cover).mp3")

		s11.volume = 0.5
		s22.volume = 0.5
		s33.volume = 0.5
		s44.volume = 0.5
		s55.volume = 0.5
		s66.volume = 0.5
		s77.volume = 0.5
		s88.volume = 0.5
		s99.volume = 0.5
		s00.volume = 0.5
		s12.volume = 0.5
		s13.volume = 0.5
		s14.volume = 0.5
		s15.volume = 0.5
		s16.volume = 0.5
		s17.volume = 0.5
		s18.volume = 0.5
		s19.volume = 0.5
		s20.volume = 0.5
		s21.volume = 0.5
		s23.volume = 0.5
		s24.volume = 0.5
		s25.volume = 0.5
		s26.volume = 0.5
		s27.volume = 0.5
		s28.volume = 0.5
		s29.volume = 0.5
		s30.volume = 0.5
		s31.volume = 0.5
		s32.volume = 0.5
		s34.volume = 0.5
		s35.volume = 0.5
		s36.volume = 0.5
		s37.volume = 0.5
		s11.preload = "auto";
		s22.preload = "auto";
		s33.preload = "auto";
		s44.preload = "auto";
		s55.preload = "auto";
		s66.preload = "auto";
		s77.preload = "auto";
		s88.preload = "auto";
		s99.preload = "auto";
		s00.preload = "auto";
		s12.preload = "auto";
		s13.preload = "auto";
		s14.preload = "auto";
		s15.preload = "auto";
		s16.preload = "auto";
		s17.preload = "auto";
		s18.preload = "auto";
		s19.preload = "auto";
		s20.preload = "auto";
		s21.preload = "auto";
		s23.preload = "auto";
		s24.preload = "auto";
		s25.preload = "auto";
		s26.preload = "auto";
		s27.preload = "auto";
		s28.preload = "auto";
		s29.preload = "auto";
		s30.preload = "auto";
		s31.preload = "auto";
		s32.preload = "auto";
		s34.preload = "auto";
		s35.preload = "auto";
		s36.preload = "auto";
		s37.preload = "auto";
		

function song1() {
	b=1
	s22.pause()

	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s11.load()
		s11.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (ခေါင်းလောင်းလေးတွေမြည်နေပြီ - အောင်ကိုလတ်)")
	
}
function song2() {
	b=4
	s11.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s22.load()
		s22.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (တန်ဖိုးထားပါ့မယ် - ကောင်းကောင်း)")
}
function song3() {
	b=7
	s11.pause()
	s22.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s33.load()
		s33.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (Yiruma, (이루마) - River Flows in You)")
}
function song4() {
	b=10
	s11.pause()
	s22.pause()
	s33.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s44.load()
		s44.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (ခင်မောင်တိုး - လွမ်း)")
}
function song5() {
	b=13
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s55.load()
		s55.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (ချစ်သူသိစေ - ခင်မောင်တိုး)")
}
function song6() {
	b=16
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s66.load()
		s66.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (ငါ့ရဲ့လျှို့ဝှက်ချက်)")
}
function song7() {
	b=19
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s77.load()
		s77.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (ဆု -ရင်ဂို)")
}
function song8() {
	b=22
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s88.load()
		s88.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (မျက်သွယ် - ခင်မောင်တိုး)")
}
function song9() {
	b=25
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s99.load()
		s99.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (ဝတ်မှုန် - ခင်မောင်တိုး)")
}
function song0() {
	b=28
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s00.load()
		s00.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (အောင်ထက် - အချစ်တေးကဗျာ)")
}
function song12() {
	b=31
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s12.load()
		s12.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing [to the bone - pamungkas (guitar cover)]")
}
function song13() {
	b=34
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s13.load()
		s13.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (ပုံရိပ် - မမြင့်)")
		
}
function song14() {
	b=37
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s14.load()
		s14.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing [ONLY - LEEHI (guitar cover)]")
}
function song15() {
	b=40
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s15.load()
		s15.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (အချစ်အယူခံ (ခင်မောင်တိုး))")
}
function song16() {
	b=43
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s16.load()
		s16.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (မောင့်ဆေးကျောင်းသူ_-_ ခင်မောင်ထူး)")
}
function song17() {
	b=46
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s17.load()
		s17.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (ဒီလိုဆိုမချစ်ဘူးကွယ် - ၀င်းဦး)")
}
function song18() {
	b=49
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
	   	s18.load()
		s18.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (ချစ်ရဲ့လားလို့ ခဏခဏမေးတဲ့ မောင့်ကိုပင်)")
}
function song19() {
	b=52
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s19.load()
		s19.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (ရုန်းတော်လွှတ်တော်-အေဝမ်းတင်မောင် & မေရှင်)")
		
}

function song20() {
	b=55
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s20.load()
		s20.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing (cant help falling in love x perfect - guitar fingerstyle cover)")
}
function song21() {
	b=58
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s21.load()
		s21.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing ချစ်သူ့ လက်ဆောင်")
} 
function song23() {
	b=61
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s23.load()
		s23.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing [at my worst - pink sweat$ (guitar fingerstyle cover)]")
}
function song24() {
	b=64
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s24.load()
		s24.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing [Until I Found You - Stephen Sanchez - Cover (Violin)]")
}
function song25() {
	b=67
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s25.load()
		s25.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing life goes on - bts (guitar fingerstyle cover)")
}
function song26() {
	b=70
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s26.load()
		s26.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing life goes on - bts (guitar fingerstyle cover)")
}
function song27() {
	b=73
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s26.pause()
	
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s27.load()
		s27.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing ရေဒီယိုကမာရွတ်")
}
function song28() {
	b=76
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s26.pause()
	s27.pause()
	
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s28.load()
		s28.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing မင်းသိဖို့ကောင်းတယ် ဇော်ဝင်းရှိန် ၁၉၈၇")
}
function song29() {
	b=79
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s29.load()
		s29.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing life goes on - bts (guitar fingerstyle cover)")
}
function song30() {
	b=82
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s30.load()
		s30.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing အချစ်မျက်ဝန်း - Lလွန်းဝါ")
}
function song31() {
	b=85
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s31.load()
		s31.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing ဒူးထောက်ပြီ - ဝိုင်ဝိုင်း")
}
function song32() {
	b=88
	b=88
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		s32.load()
		s32.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing အေးချမ်းမေ -တစ်စစီကျိုးပဲ့နေတယ်")
}
function song34() {
	b=91
	b=91
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	
	s35.pause()
	s36.pause()
	s37.pause()
		s34.load()
		s34.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing [စိုးသူ - ကြည်ဖြူပါတော့ (ကြည်ဖြူပါ မေရယ်)]")
}
function song35() {
	b=94
	b=94
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s36.pause()
	s37.pause()
		s35.load()
		s35.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing ရွှေမန်းသူ - ခင်မောင်ထူး")
}
function song36() {
	b=97
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s37.pause()
		s36.load()
		s36.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing [ငွေသော်တာရောင်ခြည် (ဆရာမ) ဘိုဘိုဟန်]")
}
function song37() {
	b=100
	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
		s37.load()
		s37.play()
		searchsong.style.display = "none"
		hi.style.filter = "none";
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("You are playing [gone - rosé (guitar fingerstyle cover)]")
}
function No() {
	a=1
		searchsong.style.display = "none"
		hi.style.filter = "none";
}
function start1() {

	s11.pause()
	s22.pause()
	s33.pause()
	s44.pause()
	s55.pause()
	s66.pause()
	s77.pause()
	s88.pause()
	s99.pause()
	s00.pause()
	s12.pause()
	s13.pause()
	s14.pause()
	s15.pause()
	s16.pause()
	s17.pause()
	s18.pause()
	s19.pause()
	s20.pause()
	s21.pause()
	s23.pause()
	s24.pause()
	s25.pause()
	s26.pause()
	s27.pause()
	s28.pause()
	s29.pause()
	s30.pause()
	s31.pause()
	s32.pause()
	s34.pause()
	s35.pause()
	s36.pause()
	s37.pause()
		stop1.style.display = "inline-block"
		start.style.display = "none"
		alert("song pause")
}
function stop11() {
	
	b++;
	if (b==2) {
		s11.play()	
	}
	else {
		s11.pause()
	}
	if (b==2) {
		b=1
	}


	if (b==5) {
		s22.play()	
	}
	else {
		s22.pause()
	}
	if (b==5) {
		b=4
	}


	if (b==8) {
		s33.play()
	} 
	else {
		s33.pause()
	}
	if (b==8) {
		b=7
	}


	if (b==11) {
		s44.play()	
	}
	else {
		s44.pause()
	}
	if (b==11) {
		b=10
	}


	if (b==14) {
		s55.play()
	}
	else {
		s55.pause()
	}
	if (b==14) {
		b=13
	}


	if (b==17) {
		s66.play()
	}
	else {
		s66.pause()
	}
	if (b==17) {
		b=16
	}


	if (b==20) {
		s77.play()
	}
	else {
		s77.pause()
	}
	if (b==20) {
		b=19
	}


	if (b==23) {
		s88.play()
	}
	else {
		s88.pause()
	}
	if (b==23) {
		b=22
	}


	if (b==26) {
		s99.play()	
	} 
	else {
		s99.pause()
	}
	if (b==26) {
		b=25
	}


	if (b==29) {
		s00.play()
	}
	else {
		s00.pause()
	}
	if (b==29) {
		b=28
	}


	if (b==32) {
		s12.play()
	}
	else {
		s12.pause()
	}
	if (b==32) {
		b=31
	}


	if (b==35) {
		s13.play()
	}
	else {
		s13.pause()
	}
	if (b==35) {
		b=34
	}


	if (b==38) {
		s14.play()
	}
	else {
		s14.pause()
	}
	if (b==38) {
		b=37
	}


	if (b==41) {
		s15.play()
	}
	else {
		s15.pause()
	}
	if (b==41) {
		b=40
	}


	if (b==44) {
		s16.play()
	}
	else {
		s16.pause()
	}
	if (b==44) {
		b=43
	}


	if (b==47) {
		s17.play()
	}
	else {
		s17.pause()
	}
	if (b==47) {
		b=46
	}


	if (b==50) {
		s18.play()
	}
	else {
		s18.pause()
	}
	if (b==50) {
		b=49
	}


	if (b==53) {
		s19.play()
	}
	else {
		s19.pause()
	}
	if (b==53) {
		b=52
	}


	if (b==56) {
		s20.play()
	}
	else {
		s20.pause()
	}
	if (b==56) {
		b=55
	}


	if (b==59) {
		s21.play()
	}
	else {
		s21.pause()
	}
	if (b==59) {
		b=58
	}


	if (b==62) {
		s23.play()
	}
	else {
		s23.pause()
	}
	if (b==62) {
		b=61
	}


	if (b==65) {
		s24.play()
	}
	else {
		s24.pause()
	}
	if (b==65) {
		b=64
	}


	if (b==68) {
		s25.play()
	}
	else {
		s25.pause()
	}
	if (b==68) {
		b=67
	}


	if (b==71) {
		s26.play()
	}
	else {
		s26.pause()
	}
	if (b==71) {
		b=70
	}


	if (b==74) {
		s27.play()
	}
	else {
		s27.pause()
	}
	if (b==74) {
		b=73
	}


	if (b==77) {
		s28.play()
	}
	else {
		s28.pause()
	}
	if (b==77) {
		b=76
	}


	if (b==80) {
		s29.play()
	}
	else {
		s29.pause()
	}
	if (b==80) {
		b=79
	}


	if (b==83) {
		s30.play()
	}
	else {
		s30.pause()
	}
	if (b==83) {
		b=82
	}


	if (b==86) {
		s31.play()
	}
	else {
		s31.pause()
	}
	if (b==86) {
		b=85
	}


	if (b==89) {
		s32.play()
	}
	else {
		s32.pause()
	}
	if (b==89) {
		b=88
	}


	if (b==92) {
		s34.play()
	}
	else {
		s34.pause()
	}
	if (b==92) {
		b=91
	}


	if (b==95) {
		s35.play()
	}
	else {
		s35.pause()
	}
	if (b==95) {
		b=94
	}


	if (b==98) {
		s36.play()
	}
	else {
		s36.pause()
	}
	if (b==98) {
		b=97
	}


	if (b==101) {
		s37.play()
	}
	else {
		s37.pause()
	}
	if (b==101) {
		b=100
	}
		start.style.display = "inline-block"
		stop1.style.display = "none"
		alert("song start playing")
}
c=1
function loop(){
	c++;
	if (c==2) {
		s11.loop = true
		s22.loop = true
		s33.loop = true
		s44.loop = true
		s55.loop = true
		s66.loop = true
		s77.loop = true
		s88.loop = true
		s99.loop = true
		s00.loop = true
		s12.loop = true
		s13.loop = true
		s14.loop = true
		s15.loop = true
		s16.loop = true
		s17.loop = true
		s18.loop = true
		s19.loop = true
		s20.loop = true
		s21.loop = true
		s23.loop = true
		s24.loop = true
		s25.loop = true
		s26.loop = true
		s27.loop = true
		s28.loop = true
		s29.loop = true
		s30.loop = true
		s31.loop = true
		s32.loop = true
		s34.loop = true
		s35.loop = true
		s36.loop = true
		s37.loop = true
		lop.style.opacity = "100%"
		alert("loop on")
	}
	else {
		s11.loop = false
		s22.loop = false
		s33.loop = false
		s44.loop = false
		s55.loop = false
		s66.loop = false
		s77.loop = false
		s88.loop = false
		s99.loop = false
		s12.loop = false
		s13.loop = false
		s15.loop = false
		s16.loop = false
		s17.loop = false
		s18.loop = false
		s19.loop = false
		s20.loop = false
		s21.loop = false
		s23.loop = false
		s24.loop = false
		s25.loop = false
		s26.loop = false
		s27.loop = false
		s28.loop = false
		s29.loop = false
		s30.loop = false
		s31.loop = false
		s32.loop = false
		s34.loop = false
		s35.loop = false
		s36.loop = false
		s37.loop = false
		lop.style.opacity = "50%"
		alert("loop off")
	}
	if (c==2) {
		c=0
	}
}
vol=5
function volup(){
	vol++;
	if (vol==1) {
		s11.volume = 0.1
		s22.volume = 0.1
		s33.volume = 0.1
		s44.volume = 0.1
		s55.volume = 0.1
		s66.volume = 0.1
		s77.volume = 0.1
		s88.volume = 0.1
		s99.volume = 0.1
		s00.volume = 0.1
		s12.volume = 0.1
		s13.volume = 0.1
		s14.volume = 0.1
		s15.volume = 0.1
		s16.volume = 0.1
		s17.volume = 0.1
		s18.volume = 0.1
		s19.volume = 0.1
		s20.volume = 0.1
		s21.volume = 0.1
		s23.volume = 0.1
		s24.volume = 0.1
		s25.volume = 0.1
		s26.volume = 0.1
		s27.volume = 0.1
		s28.volume = 0.1
		s29.volume = 0.1
		s30.volume = 0.1
		s31.volume = 0.1
		s32.volume = 0.1
		s34.volume = 0.1
		s35.volume = 0.1
		s36.volume = 0.1
		s37.volume = 0.1
	}
	if (vol==2) {
		s11.volume = 0.2
		s22.volume = 0.2
		s33.volume = 0.2
		s44.volume = 0.2
		s55.volume = 0.2
		s66.volume = 0.2
		s77.volume = 0.2
		s88.volume = 0.2
		s99.volume = 0.2
		s00.volume = 0.2
		s12.volume = 0.2
		s13.volume = 0.2
		s14.volume = 0.2
		s15.volume = 0.2
		s16.volume = 0.2
		s17.volume = 0.2
		s18.volume = 0.2
		s19.volume = 0.2
		s20.volume = 0.2
		s21.volume = 0.2
		s23.volume = 0.2
		s24.volume = 0.2
		s25.volume = 0.2
		s26.volume = 0.2
		s27.volume = 0.2
		s28.volume = 0.2
		s29.volume = 0.2
		s30.volume = 0.2
		s31.volume = 0.2
		s32.volume = 0.2
		s34.volume = 0.2
		s35.volume = 0.2
		s36.volume = 0.2
		s37.volume = 0.2
	}
	if (vol==3) {
		s11.volume = 0.3
		s22.volume = 0.3
		s33.volume = 0.3
		s44.volume = 0.3
		s55.volume = 0.3
		s66.volume = 0.3
		s77.volume = 0.3
		s88.volume = 0.3
		s99.volume = 0.3
		s00.volume = 0.3
		s12.volume = 0.3
		s13.volume = 0.3
		s14.volume = 0.3
		s15.volume = 0.3
		s16.volume = 0.3
		s17.volume = 0.3
		s18.volume = 0.3
		s19.volume = 0.3
		s20.volume = 0.3
		s21.volume = 0.3
		s23.volume = 0.3
		s24.volume = 0.3
		s25.volume = 0.3
		s26.volume = 0.3
		s27.volume = 0.3
		s28.volume = 0.3
		s29.volume = 0.3
		s30.volume = 0.3
		s31.volume = 0.3
		s32.volume = 0.3
		s34.volume = 0.3
		s35.volume = 0.3
		s36.volume = 0.3
		s37.volume = 0.3
	}
	if (vol==4) {
		s11.volume = 0.4
		s22.volume = 0.4
		s33.volume = 0.4
		s44.volume = 0.4
		s55.volume = 0.4
		s66.volume = 0.4
		s77.volume = 0.4
		s88.volume = 0.4
		s99.volume = 0.4
		s00.volume = 0.4
		s12.volume = 0.4
		s13.volume = 0.4
		s14.volume = 0.4
		s15.volume = 0.4
		s16.volume = 0.4
		s17.volume = 0.4
		s18.volume = 0.4
		s19.volume = 0.4
		s20.volume = 0.4
		s21.volume = 0.4
		s23.volume = 0.4
		s24.volume = 0.4
		s25.volume = 0.4
		s26.volume = 0.4
		s27.volume = 0.4
		s28.volume = 0.4
		s29.volume = 0.4
		s30.volume = 0.4
		s31.volume = 0.4
		s32.volume = 0.4
		s34.volume = 0.4
		s35.volume = 0.4
		s36.volume = 0.4
		s37.volume = 0.4
	}
	if (vol==5) {
		s11.volume = 0.5
		s22.volume = 0.5
		s33.volume = 0.5
		s44.volume = 0.5
		s55.volume = 0.5
		s66.volume = 0.5
		s77.volume = 0.5
		s88.volume = 0.5
		s99.volume = 0.5
		s00.volume = 0.5
		s12.volume = 0.5
		s13.volume = 0.5
		s14.volume = 0.5
		s15.volume = 0.5
		s16.volume = 0.5
		s17.volume = 0.5
		s18.volume = 0.5
		s19.volume = 0.5
		s20.volume = 0.5
		s21.volume = 0.5
		s23.volume = 0.5
		s24.volume = 0.5
		s25.volume = 0.5
		s26.volume = 0.5
		s27.volume = 0.5
		s28.volume = 0.5
		s29.volume = 0.5
		s30.volume = 0.5
		s31.volume = 0.5
		s32.volume = 0.5
		s34.volume = 0.5
		s35.volume = 0.5
		s36.volume = 0.5
		s37.volume = 0.5
	}
	if (vol==6) {
		s11.volume = 0.6
		s22.volume = 0.6
		s33.volume = 0.6
		s44.volume = 0.6
		s55.volume = 0.6
		s66.volume = 0.6
		s77.volume = 0.6
		s88.volume = 0.6
		s99.volume = 0.6
		s00.volume = 0.6
		s12.volume = 0.6
		s13.volume = 0.6
		s14.volume = 0.6
		s15.volume = 0.6
		s16.volume = 0.6
		s17.volume = 0.6
		s18.volume = 0.6
		s19.volume = 0.6
		s20.volume = 0.6
		s21.volume = 0.6
		s23.volume = 0.6
		s24.volume = 0.6
		s25.volume = 0.6
		s26.volume = 0.6
		s27.volume = 0.6
		s28.volume = 0.6
		s29.volume = 0.6
		s30.volume = 0.6
		s31.volume = 0.6
		s32.volume = 0.6
		s34.volume = 0.6
		s35.volume = 0.6
		s36.volume = 0.6
		s37.volume = 0.6
	}
	if (vol==7) {
		s11.volume = 0.7
		s22.volume = 0.7
		s33.volume = 0.7
		s44.volume = 0.7
		s55.volume = 0.7
		s66.volume = 0.7
		s77.volume = 0.7
		s88.volume = 0.7
		s99.volume = 0.7
		s00.volume = 0.7
		s12.volume = 0.7
		s13.volume = 0.7
		s14.volume = 0.7
		s15.volume = 0.7
		s16.volume = 0.7
		s17.volume = 0.7
		s18.volume = 0.7
		s19.volume = 0.7
		s20.volume = 0.7
		s21.volume = 0.7
		s23.volume = 0.7
		s24.volume = 0.7
		s25.volume = 0.7
		s26.volume = 0.7
		s27.volume = 0.7
		s28.volume = 0.7
		s29.volume = 0.7
		s30.volume = 0.7
		s31.volume = 0.7
		s32.volume = 0.7
		s34.volume = 0.7
		s35.volume = 0.7
		s36.volume = 0.7
		s37.volume = 0.7
	}
	if (vol==8) {
		s11.volume = 0.8
		s22.volume = 0.8
		s33.volume = 0.8
		s44.volume = 0.8
		s55.volume = 0.8
		s66.volume = 0.8
		s77.volume = 0.8
		s88.volume = 0.8
		s99.volume = 0.8
		s00.volume = 0.8
		s12.volume = 0.8
		s13.volume = 0.8
		s14.volume = 0.8
		s15.volume = 0.8
		s16.volume = 0.8
		s17.volume = 0.8
		s18.volume = 0.8
		s19.volume = 0.8
		s20.volume = 0.8
		s21.volume = 0.8
		s23.volume = 0.8
		s24.volume = 0.8
		s25.volume = 0.8
		s26.volume = 0.8
		s27.volume = 0.8
		s28.volume = 0.8
		s29.volume = 0.8
		s30.volume = 0.8
		s31.volume = 0.8
		s32.volume = 0.8
		s34.volume = 0.8
		s35.volume = 0.8
		s36.volume = 0.8
		s37.volume = 0.8
	}
	if (vol==9) {
		s11.volume = 0.9
		s22.volume = 0.9
		s33.volume = 0.9
		s44.volume = 0.9
		s55.volume = 0.9
		s66.volume = 0.9
		s77.volume = 0.9
		s88.volume = 0.9
		s99.volume = 0.9
		s00.volume = 0.9
		s12.volume = 0.9
		s13.volume = 0.9
		s14.volume = 0.9
		s15.volume = 0.9
		s16.volume = 0.9
		s17.volume = 0.9
		s18.volume = 0.9
		s19.volume = 0.9
		s20.volume = 0.9
		s21.volume = 0.9
		s23.volume = 0.9
		s24.volume = 0.9
		s25.volume = 0.9
		s26.volume = 0.9
		s27.volume = 0.9
		s28.volume = 0.9
		s29.volume = 0.9
		s30.volume = 0.9
		s31.volume = 0.9
		s32.volume = 0.9
		s34.volume = 0.9
		s35.volume = 0.9
		s36.volume = 0.9
		s37.volume = 0.9
	}
	if (vol==10) {
		s11.volume = 1
		s22.volume = 1
		s33.volume = 1
		s44.volume = 1
		s55.volume = 1
		s66.volume = 1
		s77.volume = 1
		s88.volume = 1
		s99.volume = 1
		s00.volume = 1
		s12.volume = 1
		s13.volume = 1
		s14.volume = 1
		s15.volume = 1
		s16.volume = 1
		s17.volume = 1
		s18.volume = 1
		s19.volume = 1
		s20.volume = 1
		s21.volume = 1
		s23.volume = 1
		s24.volume = 1
		s25.volume = 1
		s26.volume = 1
		s27.volume = 1
		s28.volume = 1
		s29.volume = 1
		s30.volume = 1
		s31.volume = 1
		s32.volume = 1
		s34.volume = 1
		s35.volume = 1
		s36.volume = 1
		s37.volume = 1
	}
	if (vol==10) {
		
		alert("this is the hightest volume")
	}
	if (vol==11) {
		vol=10
		alert("this is the hightest volume")
	}

}
function voldown() {
	vol--;
	if (vol==1) {
		s11.volume = 0.1
		s22.volume = 0.1
		s33.volume = 0.1
		s44.volume = 0.1
		s55.volume = 0.1
		s66.volume = 0.1
		s77.volume = 0.1
		s88.volume = 0.1
		s99.volume = 0.1
		s00.volume = 0.1
		s12.volume = 0.1
		s13.volume = 0.1
		s14.volume = 0.1
		s15.volume = 0.1
		s16.volume = 0.1
		s17.volume = 0.1
		s18.volume = 0.1
		s19.volume = 0.1
		s20.volume = 0.1
		s21.volume = 0.1
		s23.volume = 0.1
		s24.volume = 0.1
		s25.volume = 0.1
		s26.volume = 0.1
		s27.volume = 0.1
		s28.volume = 0.1
		s29.volume = 0.1
		s30.volume = 0.1
		s31.volume = 0.1
		s32.volume = 0.1
		s34.volume = 0.1
		s35.volume = 0.1
		s36.volume = 0.1
		s37.volume = 0.1
	}
	if (vol==2) {
		s11.volume = 0.2
		s22.volume = 0.2
		s33.volume = 0.2
		s44.volume = 0.2
		s55.volume = 0.2
		s66.volume = 0.2
		s77.volume = 0.2
		s88.volume = 0.2
		s99.volume = 0.2
		s00.volume = 0.2
		s12.volume = 0.2
		s13.volume = 0.2
		s14.volume = 0.2
		s15.volume = 0.2
		s16.volume = 0.2
		s17.volume = 0.2
		s18.volume = 0.2
		s19.volume = 0.2
		s20.volume = 0.2
		s21.volume = 0.2
		s23.volume = 0.2
		s24.volume = 0.2
		s25.volume = 0.2
		s26.volume = 0.2
		s27.volume = 0.2
		s28.volume = 0.2
		s29.volume = 0.2
		s30.volume = 0.2
		s31.volume = 0.2
		s32.volume = 0.2
		s34.volume = 0.2
		s35.volume = 0.2
		s36.volume = 0.2
		s37.volume = 0.2
	}
	if (vol==3) {
		s11.volume = 0.3
		s22.volume = 0.3
		s33.volume = 0.3
		s44.volume = 0.3
		s55.volume = 0.3
		s66.volume = 0.3
		s77.volume = 0.3
		s88.volume = 0.3
		s99.volume = 0.3
		s00.volume = 0.3
		s12.volume = 0.3
		s13.volume = 0.3
		s14.volume = 0.3
		s15.volume = 0.3
		s16.volume = 0.3
		s17.volume = 0.3
		s18.volume = 0.3
		s19.volume = 0.3
		s20.volume = 0.3
		s21.volume = 0.3
		s23.volume = 0.3
		s24.volume = 0.3
		s25.volume = 0.3
		s26.volume = 0.3
		s27.volume = 0.3
		s28.volume = 0.3
		s29.volume = 0.3
		s30.volume = 0.3
		s31.volume = 0.3
		s32.volume = 0.3
		s34.volume = 0.3
		s35.volume = 0.3
		s36.volume = 0.3
		s37.volume = 0.3
	}
	if (vol==4) {
		s11.volume = 0.4
		s22.volume = 0.4
		s33.volume = 0.4
		s44.volume = 0.4
		s55.volume = 0.4
		s66.volume = 0.4
		s77.volume = 0.4
		s88.volume = 0.4
		s99.volume = 0.4
		s00.volume = 0.4
		s12.volume = 0.4
		s13.volume = 0.4
		s14.volume = 0.4
		s15.volume = 0.4
		s16.volume = 0.4
		s17.volume = 0.4
		s18.volume = 0.4
		s19.volume = 0.4
		s20.volume = 0.4
		s21.volume = 0.4
		s23.volume = 0.4
		s24.volume = 0.4
		s25.volume = 0.4
		s26.volume = 0.4
		s27.volume = 0.4
		s28.volume = 0.4
		s29.volume = 0.4
		s30.volume = 0.4
		s31.volume = 0.4
		s32.volume = 0.4
		s34.volume = 0.4
		s35.volume = 0.4
		s36.volume = 0.4
		s37.volume = 0.4
	}
	if (vol==5) {
		s11.volume = 0.5
		s22.volume = 0.5
		s33.volume = 0.5
		s44.volume = 0.5
		s55.volume = 0.5
		s66.volume = 0.5
		s77.volume = 0.5
		s88.volume = 0.5
		s99.volume = 0.5
		s00.volume = 0.5
		s12.volume = 0.5
		s13.volume = 0.5
		s14.volume = 0.5
		s15.volume = 0.5
		s16.volume = 0.5
		s17.volume = 0.5
		s18.volume = 0.5
		s19.volume = 0.5
		s20.volume = 0.5
		s21.volume = 0.5
		s23.volume = 0.5
		s24.volume = 0.5
		s25.volume = 0.5
		s26.volume = 0.5
		s27.volume = 0.5
		s28.volume = 0.5
		s29.volume = 0.5
		s30.volume = 0.5
		s31.volume = 0.5
		s32.volume = 0.5
		s34.volume = 0.5
		s35.volume = 0.5
		s36.volume = 0.5
		s37.volume = 0.5
	}
	if (vol==6) {
		s11.volume = 0.6
		s22.volume = 0.6
		s33.volume = 0.6
		s44.volume = 0.6
		s55.volume = 0.6
		s66.volume = 0.6
		s77.volume = 0.6
		s88.volume = 0.6
		s99.volume = 0.6
		s00.volume = 0.6
		s12.volume = 0.6
		s13.volume = 0.6
		s14.volume = 0.6
		s15.volume = 0.6
		s16.volume = 0.6
		s17.volume = 0.6
		s18.volume = 0.6
		s19.volume = 0.6
		s20.volume = 0.6
		s21.volume = 0.6
		s23.volume = 0.6
		s24.volume = 0.6
		s25.volume = 0.6
		s26.volume = 0.6
		s27.volume = 0.6
		s28.volume = 0.6
		s29.volume = 0.6
		s30.volume = 0.6
		s31.volume = 0.6
		s32.volume = 0.6
		s34.volume = 0.6
		s35.volume = 0.6
		s36.volume = 0.6
		s37.volume = 0.6
	}
	if (vol==7) {
		s11.volume = 0.7
		s22.volume = 0.7
		s33.volume = 0.7
		s44.volume = 0.7
		s55.volume = 0.7
		s66.volume = 0.7
		s77.volume = 0.7
		s88.volume = 0.7
		s99.volume = 0.7
		s00.volume = 0.7
		s12.volume = 0.7
		s13.volume = 0.7
		s14.volume = 0.7
		s15.volume = 0.7
		s16.volume = 0.7
		s17.volume = 0.7
		s18.volume = 0.7
		s19.volume = 0.7
		s20.volume = 0.7
		s21.volume = 0.7
		s23.volume = 0.7
		s24.volume = 0.7
		s25.volume = 0.7
		s26.volume = 0.7
		s27.volume = 0.7
		s28.volume = 0.7
		s29.volume = 0.7
		s30.volume = 0.7
		s31.volume = 0.7
		s32.volume = 0.7
		s34.volume = 0.7
		s35.volume = 0.7
		s36.volume = 0.7
		s37.volume = 0.7
	}
	if (vol==8) {
		s11.volume = 0.8
		s22.volume = 0.8
		s33.volume = 0.8
		s44.volume = 0.8
		s55.volume = 0.8
		s66.volume = 0.8
		s77.volume = 0.8
		s88.volume = 0.8
		s99.volume = 0.8
		s00.volume = 0.8
		s12.volume = 0.8
		s13.volume = 0.8
		s14.volume = 0.8
		s15.volume = 0.8
		s16.volume = 0.8
		s17.volume = 0.8
		s18.volume = 0.8
		s19.volume = 0.8
		s20.volume = 0.8
		s21.volume = 0.8
		s23.volume = 0.8
		s24.volume = 0.8
		s25.volume = 0.8
		s26.volume = 0.8
		s27.volume = 0.8
		s28.volume = 0.8
		s29.volume = 0.8
		s30.volume = 0.8
		s31.volume = 0.8
		s32.volume = 0.8
		s34.volume = 0.8
		s35.volume = 0.8
		s36.volume = 0.8
		s37.volume = 0.8
	}
	if (vol==9) {
		s11.volume = 0.9
		s22.volume = 0.9
		s33.volume = 0.9
		s44.volume = 0.9
		s55.volume = 0.9
		s66.volume = 0.9
		s77.volume = 0.9
		s88.volume = 0.9
		s99.volume = 0.9
		s00.volume = 0.9
		s12.volume = 0.9
		s13.volume = 0.9
		s14.volume = 0.9
		s15.volume = 0.9
		s16.volume = 0.9
		s17.volume = 0.9
		s18.volume = 0.9
		s19.volume = 0.9
		s20.volume = 0.9
		s21.volume = 0.9
		s23.volume = 0.9
		s24.volume = 0.9
		s25.volume = 0.9
		s26.volume = 0.9
		s27.volume = 0.9
		s28.volume = 0.9
		s29.volume = 0.9
		s30.volume = 0.9
		s31.volume = 0.9
		s32.volume = 0.9
		s34.volume = 0.9
		s35.volume = 0.9
		s36.volume = 0.9
		s37.volume = 0.9
	}
	if (vol==0) {
		s11.volume = 0
		s22.volume = 0
		s33.volume = 0
		s44.volume = 0
		s55.volume = 0
		s66.volume = 0
		s77.volume = 0
		s88.volume = 0
		s99.volume = 0
		s00.volume = 0
		s12.volume = 0
		s13.volume = 0
		s14.volume = 0
		s15.volume = 0
		s16.volume = 0
		s17.volume = 0
		s18.volume = 0
		s19.volume = 0
		s20.volume = 0
		s21.volume = 0
		s23.volume = 0
		s24.volume = 0
		s25.volume = 0
		s26.volume = 0
		s27.volume = 0
		s28.volume = 0
		s29.volume = 0
		s30.volume = 0
		s31.volume = 0
		s32.volume = 0
		s34.volume = 0
		s35.volume = 0
		s36.volume = 0
		s37.volume = 0
	}
	if (vol==0) {
		
		alert("this is the lowest volume")
	}
	if (vol==-1) {
		vol = 0
		alert("this is the hightest volume")
	}
}